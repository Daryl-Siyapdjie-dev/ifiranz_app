...
class OrderDeliveryTabScreen extends StatefulHookConsumerWidget {
  ...
  @override
  ConsumerState<ConsumerStatefulWidget> createState() => _OrderDeliveryTabScreenState();
}

class _OrderDeliveryTabScreenState extends ConsumerState<OrderDeliveryTabScreen> {
      ...
      if (controller.position.maxScrollExtent == controller.position.pixels) {
        if (ref.watch(deliveryOrdersNotifierProvider).hasValue &&
          ...
          await ref
              ...
              .whenComplete(() {
            if (!(ref.watch(deliveryOrdersNotifierProvider).value!.hasErrorOnLoadMore)) {
              setState(() {
        ...
        child: ProgressHUD(
            ...
            child: Builder(builder: (_) {
              return RefreshIndicator(
                onRefresh: () {
                  return ref.refresh(deliveryOrdersNotifierProvider.notifier).fetchDeliveryOrders(PaginatedRequest(size: params.size));
                },
                child: Scaffold(
                    key: _scaffoldKey,
                    appBar: CustomDeliveryAppBar(
                                ...
                                title: TextField(
                                  ...
                                  onEditingComplete: () async {
                                    if (searchInputController.text.isNotEmpty) {
                                      ...
                                      progress?.show();
                                      await ref.read(deliveryOrdersNotifierProvider.notifier).findOrders([
                                        FilterOptional(
                                          type: "EQUAL",
                                          key: "localisationGps",
                                          value: searchInputController.text.trim(),
                                        ),
                                  ...
                                  decoration: InputDecoration(hintText: context.locale.search, prefixIcon: const Icon(Icons.search)),
                                ),
                                actions: [
                                  IconButton(
                                      onPressed: () {
                        ...
                        action: Row(
                          children: [
                            ...
                            IconButton(
                                onPressed: () async {
                                  final bool? res = await showDialog(context: context, builder: (context) => const SearchFilterWidget());
                ...
                children: [
                  ...
                  Row(
                    children: [
                      ...
                      gapW10,
                      Expanded(
                        child: Text('${record.commande?.localisationGps}'.capitalize(),
                  ...
                  children: [
                    ElevatedButton(
                        style: ElevatedButton.styleFrom(
                            backgroundColor: AppColors.variantGreenColor, padding: const EdgeInsets.symmetric(horizontal: 4, vertical: 9)),
                        onPressed: () async {
                          final progress = ProgressHUD.of(_);
                          progress?.show();

                          ref.read(deliveryOrdersNotifierProvider.notifier).acceptDeliveryOrder(record.id!).whenComplete(() => progress?.dismiss());
                        },
                        child: Text(
                    ...
                    TextButton(
                        style: ElevatedButton.styleFrom(padding: const EdgeInsets.symmetric(horizontal: 4, vertical: 9)),
                        onPressed: () async {
                          final progress = ProgressHUD.of(_);
                          progress?.show();

                          ref.read(deliveryOrdersNotifierProvider.notifier).denyDeliveryOrder(record.id!).whenComplete(() => progress?.dismiss());
                        },
                        child: Text(
                          context.locale.deny,
                          style: const TextStyle(fontSize: 10, color: AppColors.alertError),
                        )),
                  ],
                ),
              )
                        ...
                        children: [
                          ...
                          ElevatedButton(
                              onPressed: () {
                                final progress = ProgressHUD.of(_);
                                progress?.show();
                                ref.read(deliveryOrdersNotifierProvider.notifier).findOrders([
                                  FilterOptional(
                                    type: "EQUAL",
                                    key: "statut",
                                    value: status!.value,
