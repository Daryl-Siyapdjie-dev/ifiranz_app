...
class InfrastructursRemoteService {
  ...
  Future<ApiResponse<Json>> getInfrastructurs(PaginatedRequest params, [List<FilterOptional> request = const []]) async {
    return handleApiCall<ApiResponse<Json>>(
      () async => _dio.post(_urlBuilder.buildInfrastructur(params), data: request.map((e) => e.toJson()).toList()),
      (data) {
  ...
  Future<ApiResponse<Json>> getTop10Infrastructurs(PaginatedRequest params, [List<FilterOptional> request = const []]) async {
    return handleApiCall<ApiResponse<Json>>(
      () async => _dio.get(
