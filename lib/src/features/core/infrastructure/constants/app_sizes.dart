// Tailles utilisées pour les paddings, sizedbox, bordures
import 'package:flutter/cupertino.dart';

class AppSizes {
  static const double p2 = 2;
  static const double p4 = 4;
  static const double p5 = 5;
  static const double p6 = 6;
  static const double p8 = 8;
  static const double p10 = 10;
  static const double p12 = 12;
  static const double p16 = 16;
  static const double p18 = 18;
  static const double p20 = 20;
  static const double p25 = 25;
  static const double p30 = 30;
  static const double p32 = 32;
  static const double p64 = 64;
  static const double p44 = 44;
  static const double p48 = 48;
  static const double pDesignAppBarWidth = 393;
  static const double pDesignAppBarHeight = 94;
  static const double pDesignHeight = 852;
}

// SizedBox Height
const gapH2 = SizedBox(height: AppSizes.p2);
const gapH4 = SizedBox(height: AppSizes.p4);
const gapH6 = SizedBox(height: AppSizes.p6);
const gapH8 = SizedBox(height: AppSizes.p8);
const gapH10 = SizedBox(height: AppSizes.p10);
const gapH12 = SizedBox(height: AppSizes.p12);
const gapH16 = SizedBox(height: AppSizes.p16);
const gapH18 = SizedBox(height: AppSizes.p18);
const gapH20 = SizedBox(height: AppSizes.p20);
const gapH25 = SizedBox(height: AppSizes.p25);
const gapH30 = SizedBox(height: AppSizes.p30);
const gapH32 = SizedBox(height: AppSizes.p32);
const gapH64 = SizedBox(height: AppSizes.p64);

// SizedBox Width
const gapW2 = SizedBox(width: AppSizes.p2);
const gapW4 = SizedBox(width: AppSizes.p4);
const gapW5 = SizedBox(width: AppSizes.p4);
const gapW6 = SizedBox(width: AppSizes.p6);
const gapW8 = SizedBox(width: AppSizes.p8);
const gapW10 = SizedBox(width: AppSizes.p10);
const gapW12 = SizedBox(width: AppSizes.p12);
const gapW16 = SizedBox(width: AppSizes.p16);
const gapW18 = SizedBox(width: AppSizes.p18);
const gapW20 = SizedBox(width: AppSizes.p20);
const gapW25 = SizedBox(width: AppSizes.p25);
const gapW30 = SizedBox(width: AppSizes.p30);
const gapW32 = SizedBox(width: AppSizes.p32);
const gapW64 = SizedBox(width: AppSizes.p64);

//Radius
Radius rad16 = const Radius.circular(16);
